...
namespace DbWrapper
{
    ...
    public class Record : FlexObject
    {
        ...
        public Record(string table, Database db, uint pageSize) : base(pageSize)
        {
            _database = db;
            _table = table;
            _query = new Query(db);
            ...
            foreach (var column in base.PropertyListInfo.Keys)
            {
                ...
                if (t.IsValueType)
                {
                    if (t == typeof(DateTime))
                        this.PropertyList.Add(column, SqlDateTime.MinValue);
                    else
                        this.PropertyList.Add(column, Activator.CreateInstance(t));
                }
                else if (!t.IsValueType && t == typeof(byte[]))
                {
                    this.PropertyList.Add(column, new byte[0]);
                }
                else
                {
                    this.PropertyList.Add(column, DBNull.Value);
                }
            }
        }
        /// <summary>
                                      ...
                                      int index)
        {
            for (int j = 0; j < columns.Count; j++)
            {
                ...
                _recordList[index].PropertyList.Add(col.ColumnName, val);
            }
        }
        ...
        private bool NextRecord()
        {
            ...
            {
                this.PropertyList = _recordList[_recordIndex].PropertyList;
                return true;
            }
            else
                return false;
        ...
        public void AddJoin(string joinTable,
                            string joinColumn,
                            string joinOn,
        ...
        {
            int index = base.IdentityColumns.FindIndex(x => x.Column.Equals(column));
